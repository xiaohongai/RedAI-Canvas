...
## 🧩 本地 ComfyUI（无需 API Key）

RedAI-Canvas 支持连接本机或局域网内的 **ComfyUI** 后端，导入 API 工作流后在画布上直接出图。
...
4. **暴露参数**：勾选需要出现在节点上的字段（提示词、步数、尺寸等），设置显示名称与控件类型
...
**查看当前版本**：应用界面右上角版本徽章，或 `index.html` 中的 `app-version` meta 标签。
