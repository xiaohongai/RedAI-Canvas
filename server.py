...
DEFAULT_COMFYUI_WORKFLOWS_DIR = _get_path_env(
    "AIC_COMFYUI_WORKFLOWS_DIR",
    os.path.join(DEFAULT_DATA_DIR, "comfyui", "workflows"),
)
...
COMFYUI_WORKFLOWS_DIR = DEFAULT_COMFYUI_WORKFLOWS_DIR
...
def _refresh_storage_globals(paths):
    ...
    global WORKFLOWS_DIR, WORKFLOW_THUMBS_DIR, COMFYUI_WORKFLOWS_DIR, OUTPUT_DIR
    global CONFIG_FILE, SETTINGS_FILE, GEN_SEQ_STATE_FILE
    global DREAMINA_CLI_SERVICE, DREAMINA_ROUTE_SERVICE
    ...
    WORKFLOW_THUMBS_DIR = os.path.join(ASSETS_DIR, "workflows", "thumbs")
    COMFYUI_WORKFLOWS_DIR = os.path.join(DATA_DIR, "comfyui", "workflows")
...
def _seed_comfyui_workflows():
    seed_src = os.path.join(DIRECTORY, "workflows", "image_z_image_turbo-api.json")
    seed_dst_dir = os.path.join(COMFYUI_WORKFLOWS_DIR, "custom")
    seed_dst = os.path.join(seed_dst_dir, "image_z_image_turbo-api.json")
...
COMFYUI_ROUTE_SERVICE = ComfyuiRouteService(
    workflows_dir_getter=lambda: COMFYUI_WORKFLOWS_DIR,
    output_dir_getter=lambda: OUTPUT_DIR,
...
def _normalize_request_path(raw_path):
    path = str(raw_path or "").split("?", 1)[0]
    if path != "/":
        path = path.rstrip("/")
    return path or "/"
...
HTTP_ROUTE_DISPATCHER = HttpRouteDispatcher(
    ...
    comfyui_route_service_getter=lambda: COMFYUI_ROUTE_SERVICE,
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_PUT(self):
        ...
        if not _enforce_local_api_access(self, path):
            return
        if HTTP_ROUTE_DISPATCHER.handle_put(self, path):
            return
    ...
    def end_headers(self):
        ...
        if not has_server_id:
            self.send_header("X-AICanvas-Server", "RedAI-Canvas")
