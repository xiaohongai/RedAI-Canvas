...
class HttpRouteDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        self._get_comfyui_route_service = comfyui_route_service_getter
    ...
    def handle_get(self, handler, path):
        ...
        comfyui_get_response = self._get_comfyui_route_service().handle_get(
            handler,
            path,
        )
        if comfyui_get_response is not None:
            self._send_route_response(handler, comfyui_get_response)
            return True
    ...
    def handle_post(self, handler, path):
        ...
        comfyui_post_paths = (
            ...
            "/api/v2/comfyui/upload",
            "/api/v2/comfyui/generate",
        )
        comfyui_post_response = self._get_comfyui_route_service().handle_post(
            handler,
            path,
            self._read_body(handler)
            if (
                path in comfyui_post_paths
                or path.startswith("/api/v2/comfyui/workflows/")
            )
            else b"",
        )
        if comfyui_post_response is not None:
            self._send_route_response(handler, comfyui_post_response)
            return True
    ...
    def handle_put(self, handler, path):
        comfyui_put_response = self._get_comfyui_route_service().handle_put(
            handler,
            path,
            self._read_body(handler) if path.startswith("/api/v2/comfyui/") else b"",
        )
        if comfyui_put_response is not None:
            self._send_route_response(handler, comfyui_put_response)
            return True
        return False

    def handle_delete(self, handler, path):
        comfyui_delete_response = self._get_comfyui_route_service().handle_delete(
            handler,
            path,
        )
        if comfyui_delete_response is not None:
            self._send_route_response(handler, comfyui_delete_response)
            return True
